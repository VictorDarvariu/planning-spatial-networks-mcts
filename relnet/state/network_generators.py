...
class RealWorldNetworkGenerator(OrdinaryGraphGenerator, ABC):
    def __init__(self, store_graphs=False, graph_storage_root=None, logs_file=None, original_dataset_dir=None):
        ...
        with open(graph_metadata_file.resolve(), "r") as fh:
            ...
            self.num_graphs, \
            self.graph_names,\
            self.graph_props = graph_metadata['num_graphs'], graph_metadata['graph_names'], graph_metadata['graph_props']
...
class InternetTopologyNetworkGenerator(RealWorldNetworkGenerator):
    name = InternetTopologyDataPreprocessor.DS_NAME


class MetroNetworkGenerator(RealWorldNetworkGenerator):
    name = MetroDataPreprocessor.DS_NAME
