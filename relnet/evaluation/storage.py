...
class EvaluationStorage:
    MONGO_EXPERIMENT_COLLECTION = {'model': 'experiment_data',
                                   'planning': 'experiment_data_planning'}
    MONGO_EVALUATION_COLLECTION = {'model': 'evaluation_data',
                                   'planning': 'evaluation_data_planning'}
    ...
    def find_latest_experiment_id(self, algorithm_class, edge_percentage):
        result = self.db[self.MONGO_EXPERIMENT_COLLECTION[algorithm_class]].find(
            {"experiment_conditions.possible_edge_percentage" : edge_percentage}
        ).sort([("started_millis", -1)]) \
        .limit(1)[0]["experiment_id"]
        return result

    def get_hyperparameter_optimisation_data(self, algorithm_class,
                                     experiment_id,
                                     model_seeds_to_skip,
                                     train_individually):

        latest_experiment = self.get_experiment_details(algorithm_class, experiment_id)
    ...
    def retrieve_optimal_hyperparams(self, algorithm_class,
                                     experiment_id,
                                     model_seeds_to_skip,
                                     train_individually):

        param_spaces, df = self.get_hyperparameter_optimisation_data(algorithm_class,
                                                                     experiment_id,
                                                                     model_seeds_to_skip,
                                                                     train_individually)
    ...
    def get_evaluation_data(self, algorithm_class, experiment_id):
        eval_data = self.db[self.MONGO_EVALUATION_COLLECTION[algorithm_class]].find({"experiment_id": experiment_id})
        all_results_rows = []
        for eval_item in eval_data:
            all_results_rows.extend(list(eval_item['results_rows']))
        return all_results_rows

    def remove_evaluation_data(self, algorithm_class, experiment_id):
        self.db[self.MONGO_EVALUATION_COLLECTION[algorithm_class]].remove({"experiment_id": experiment_id})
    ...
    def insert_experiment_details(self,
        ...
        self.db[self.MONGO_EXPERIMENT_COLLECTION[algorithm_class]].insert_one(all_experiment_details)

        return all_experiment_details

    def get_experiment_details(self, algorithm_class, experiment_id):
        return self.db[self.MONGO_EXPERIMENT_COLLECTION[algorithm_class]].find(
            {"experiment_id": {"$eq": experiment_id}}).limit(1)[0]

    def update_with_hyperopt_results(self, algorithm_class, experiment_id, optimisation_result):
        self.db[self.MONGO_EXPERIMENT_COLLECTION[algorithm_class]].update_one({"experiment_id": experiment_id},
                                                                   {"$set": {"optimisation_result": optimisation_result}})

    def insert_evaluation_results(self, algorithm_class, experiment_id, results_rows):
        self.db[self.MONGO_EVALUATION_COLLECTION[algorithm_class]].insert_one({"experiment_id": experiment_id,
                                                                "results_rows": results_rows})

    def delete_evaluation_results(self, algorithm_class, experiment_id):
        n_deleted_rows = self.db[self.MONGO_EVALUATION_COLLECTION[algorithm_class]].delete_many({"experiment_id": experiment_id})
        print(f"deleted {n_deleted_rows}.")


    def fetch_all_eval_curves(self, agent_name, hyperparams_id, file_paths, objective_functions, network_generators, model_seeds, train_individually, use_test=False):
        all_dfs = []
        for obj_fun_name in objective_functions:
            for net_gen_name in network_generators:
                all_dfs.append(self.fetch_eval_curves(agent_name, hyperparams_id, file_paths, obj_fun_name, net_gen_name, model_seeds, train_individually, use_test))
        return pd.concat(all_dfs)

    def fetch_eval_curves(self, agent_name, hyperparams_id, file_paths, objective_function, network_generator, model_seeds, train_individually, use_test):
        if use_test:
            which = 'histories_test'
        else:
            which = 'histories_eval'
        return self.fetch_step_data(which, agent_name, hyperparams_id, file_paths, objective_function, network_generator, model_seeds, train_individually)

    def fetch_step_data(self, which, agent_name, hyperparams_id, file_paths, objective_function, network_generator, model_seeds, train_individually):
        if which == 'histories_eval':
            file_dir = file_paths.eval_histories_dir
            header_names = ['timestep', 'algorithm', 'perf']
            fname_method = file_paths.construct_history_file_name

        elif which == 'histories_test':
            file_dir = file_paths.test_histories_dir
            header_names = ['timestep', 'algorithm', 'perf']
            fname_method = file_paths.construct_history_file_name

        elif which == 'timings':
            file_dir = file_paths.timings_dir
            header_names = ['timestep', 'unix_ms']
            fname_method = file_paths.construct_timings_file_name
        else:
            raise ValueError(f"data qualifier {which} not recognised.")

        if len(list(file_dir.iterdir())) == 0:
            return pd.DataFrame()
        ...
        for seed in model_seeds:
            ...
            for idx, g_id in enumerate(graph_ids):
                model_identifier_prefix = file_paths.construct_model_identifier_prefix(agent_name, objective_function,
                                                                                       network_generator, seed,
                                                                                       hyperparams_id, graph_id=g_id)
                filename = fname_method(model_identifier_prefix)
                data_file = file_dir / filename
                if data_file.exists():
                    eval_df = pd.read_csv(data_file, sep=",", header=None, names=header_names)
